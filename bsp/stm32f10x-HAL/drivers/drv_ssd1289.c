...
void lcd_disp_flush(int32_t x1, int32_t y1, int32_t x2, int32_t y2, const lv_color_t* color_p)
{
	for(int32_t x = x1; x <= x2; x++) {
		for(int32_t y = y1; y <= y2; y++) {
			/* Put a pixel to the display. For example: */
			/* put_px(x, y, *color_p)*/
			ssd1289_lcd_set_pixel(x, y, color_p->full);
			color_p++;
		}
	}
	/* IMPORTANT!!!
 ...
 * This function is required only when LV_VDB_SIZE == 0 in lv_conf.h*/
void lcd_disp_map(int32_t x1, int32_t y1, int32_t x2, int32_t y2, const lv_color_t* color_p)
{
	for(int32_t x = x1; x <= x2; x++) {
		for(int32_t y = y1; y <= y2; y++) {
			/* Put a pixel to the display. For example: */
...
/* Write a pixel array (called 'map') to the a specific area on the display
 * This function is required only when LV_VDB_SIZE == 0 in lv_conf.h*/
void lcd_disp_fill(int32_t x1, int32_t y1, int32_t x2, int32_t y2,  lv_color_t color)
{
    ...
    for(x = x1; x <= x2; x++) {
        for(y = y1; y <= y2; y++) {
            /* Put a pixel to the display. For example: */
            /* put_px(x, y, *color)*/
 					ssd1289_lcd_set_pixel(x, y, color.full);
        }
    }
}
