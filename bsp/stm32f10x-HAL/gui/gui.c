...
UG_S16 UG_Init( UG_GUI* g, void (*p)(UG_S16,UG_S16,UG_COLOR), UG_S16 x, UG_S16 y )
{
   ...
   g->pset = (void(*)(const char*,UG_S16,UG_S16))p;
   g->x_dim = x;
   g->y_dim = y;
   g->console.x_start = 4;
   g->console.y_start = 4;
...
void UG_DrawCircle( UG_S16 x0, UG_S16 y0, UG_S16 r, UG_COLOR c )
{
   ...
   while ( x >= y )
   {
      gui->pset((const char*)(&c), x0 - x, y0 + y);
      gui->pset((const char*)(&c), x0 - x, y0 - y);
      gui->pset((const char*)(&c), x0 + x, y0 + y);
      gui->pset((const char*)(&c), x0 + x, y0 - y);
      gui->pset((const char*)(&c), x0 - y, y0 + x);
      gui->pset((const char*)(&c), x0 - y, y0 - x);
      gui->pset((const char*)(&c), x0 + y, y0 + x);
      gui->pset((const char*)(&c), x0 + y, y0 - x);
...
void UG_DrawArc( UG_S16 x0, UG_S16 y0, UG_S16 r, UG_U8 s, UG_COLOR c )
{
   ...
   {
      // Q1
      if ( s & 0x01 ) gui->pset((const char*)(&c), x0 + x, y0 - y);
      if ( s & 0x02 ) gui->pset((const char*)(&c), x0 + y, y0 - x);

      // Q2
      if ( s & 0x04 ) gui->pset((const char*)(&c), x0 - y, y0 - x);
      if ( s & 0x08 ) gui->pset((const char*)(&c), x0 - x, y0 - y);

      // Q3
      if ( s & 0x10 ) gui->pset((const char*)(&c), x0 - x, y0 + y);
      if ( s & 0x20 ) gui->pset((const char*)(&c), x0 - y, y0 + x);

      // Q4
      if ( s & 0x40 ) gui->pset((const char*)(&c), x0 + y, y0 + x);
      if ( s & 0x80 ) gui->pset((const char*)(&c), x0 + x, y0 + y);
         ...
         for( j=0;j<font->char_height;j++ )
         {
           ...
           for( i=0;i<bn;i++ )
           {
             ...
             for( k=0;(k<8) && c;k++ )
             {
               if( b & 0x01 )
               {
                  gui->pset((const char*)(&fc),xo,yo);
               }
               else
               {
                  gui->pset((const char*)(&bc),xo,yo);
               }
               b >>= 1;
               xo++;
               c--;
         ...
         for( j=0;j<font->char_height;j++ )
         {
            ...
            for( i=0;i<actual_char_width;i++ )
            {
               ...
               color = (((fc & 0xFF) * b + (bc & 0xFF) * (256 - b)) >> 8) & 0xFF |//Blue component
                       (((fc & 0xFF00) * b + (bc & 0xFF00) * (256 - b)) >> 8)  & 0xFF00|//Green component
                       (((fc & 0xFF0000) * b + (bc & 0xFF0000) * (256 - b)) >> 8) & 0xFF0000; //Red component
               gui->pset((const char*)(&color),xo,yo);
               xo++;
            }
            index += font->char_width - actual_char_width;
            yo++;
